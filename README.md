...
1. **仓库设为私有**  
   既然只是原型期展示，暂时不需要公开。在仓库 `Settings` → `Danger Zone` → `Change visibility` → `Make private`。

2. **考虑是否移除 `xirang_submission.zip`**  
   它虽无核心代码，但包含完整证明链文本和软著摘要，如果仓库短期内保持公开，建议也一并删除或放入单独的私有发布页。等以后正式开源时再放白皮书不迟。
